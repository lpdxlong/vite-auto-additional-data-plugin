 ...
 */
export function additionalData(options: Options): Plugin<any> {
	// 将选项中的键转换为正则表达式对象，并存储匹配信息。
	let matches = Object.keys(options).map((key) => {
		return {
	...
	return {
		...
		transform(code, id) {
			...
			// 获取匹配规则对应的插入数据。
			let insertCodes = options[match.key];
